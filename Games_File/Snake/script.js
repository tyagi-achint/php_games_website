...
if (document.getElementById("snake_game")) {
  pName = "Anonymous";
  ...
  const grid = document.querySelector("#snake_game .grid");
  ...
  const tiles = document.querySelectorAll("#snake_game .grid .tile .content");

  const containerElement = document.querySelector("#snake_game .container");
  const noteElement = document.querySelector("#snake_game .snake_game h2");
  const contrastElement = document.querySelector("#snake_game .contrast");
  const scoreElement = document.querySelector("#snake_game .score");
  ...
  function resetGame() {
    ...
    if (hardMode) {
      scoreElement.innerText = "H " + score;
    }
    else {
      scoreElement.innerText = "E " + score;
    }
    ...
    try {
      ...
      if (stepsTaken != stepsShouldHaveTaken) {
        ...
        if (headPosition == applePosition) {
          ...
          if (hardMode) {
            finalScore = score * 10;
            scoreElement.innerText = "H " + finalScore;
          }
          else {
            finalScore = score * 5;
            scoreElement.innerText = "E " + finalScore;
          }
      ...
      if (finalScore > 30) {
        saveScoreToDatabase(finalScore, pName)
      }
      noteElement.style.opacity = 1;
  ...
  var howToPlay = document.getElementById("howtoMenu");
  howToPlay.addEventListener("click", function () {
    document.querySelector(".snake_game .container").classList.toggle("contentHide");
    document.querySelector(".howToPlay").classList.toggle("contentHide");
    howToPlay.innerText = (howToPlay.innerText === "How to Play?") ? "Back to Game" : "How to Play?";
  });
